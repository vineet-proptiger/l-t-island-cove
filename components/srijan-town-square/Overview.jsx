...
import { PROJECT_ID, PROJECT_NAME, API_ENDPOINT, SHEET_NAME, SECRET_KEY, CITY_DISPLAY } from '../../lib/srijan-town-config'
import { getGeo, buildTrackingFields } from '../../lib/formMeta'
import { overviewImage } from '../../lib/srijan-town-images'
...
const PREVIEW_TEXT = `Buy a new superspacious luxurious living in an unmatched location of Newtown Kolkata. You are welcome to Town Square, a signature project beside Novotel Crossing, Action Area 1, Newtown. Roads on all 4 sides of this project. Landscapes spreading over 6.29 acres. Enjoy 50+ lifestyle amenities.`

const MORE_TEXT = ` It has total 7 Towers with 350 super-spacious apartments, all incomparably airy and open on 3 sides.`

const OPEN_VIEWS = [
  { dir: 'E', label: 'Biswa Bangla', sub: 'East' },
  { dir: 'N', label: 'Ecopark', sub: 'North' },
  { dir: 'SW', label: 'Wetlands', sub: 'South-West' },
  { dir: 'NW', label: 'Airport', sub: 'North-West' },
  { dir: 'W·S', label: 'City', sub: 'West & South' },
]
    ...
    <section id="overview" className="!pt-1 !pb-4 sm:!py-10" style={{
      ...
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div style={{ textAlign: 'center', marginBottom: '36px' }} data-aos="fade-up">
          ...
          }}>Srijan Town Square New Launch</span>
          <h2 style={{
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
          ...
          <div data-aos="fade-right" style={{ display: 'flex' }}>
            <div style={{
              ...
              <Image src={overviewImage} alt="Srijan Town Square" fill className="object-contain"
              ...
              <div style={{ borderTop: '1px solid #f0f0f0', paddingTop: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
                  <span style={{
                    ...
                    color: '#fff', fontFamily: F_JOST, fontSize: '13px', fontWeight: '800',
                    padding: '3px 10px', borderRadius: '6px', letterSpacing: '0.04em',
                  }}>270°</span>
                  <span style={{ fontFamily: F_JOST, fontSize: '15px', fontWeight: '800', color: 'var(--color-dark)' }}>Open View</span>
                  <span style={{ flex: 1, height: '1px', background: 'linear-gradient(90deg, var(--color-gold-light), transparent)' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                  {OPEN_VIEWS.map((v, i, arr) => (
                    <div key={i} style={{
                      display: 'flex', alignItems: 'center', gap: '12px',
                      padding: '8px 0',
                      borderBottom: i < arr.length - 1 ? '1px dashed #f0ebe0' : 'none',
                    }}>
                      <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--color-gold)', flexShrink: 0 }} />
                      <span style={{ fontFamily: F_JOST, fontSize: '13px', fontWeight: '700', color: 'var(--color-dark)' }}>{v.label}</span>
                      <span style={{
                        marginLeft: 'auto', fontFamily: F_SANS, fontSize: '10px',
                        color: '#9ca3af', fontWeight: '600', letterSpacing: '0.04em',
                      }}>{v.sub}</span>
                    </div>
                  ))}
                </div>
