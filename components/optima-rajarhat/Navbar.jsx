...
const Navbar = ({ setIsOpen }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-[var(--color-bg)] transition-shadow duration-300 ${scrolled ? 'shadow-md' : 'shadow-sm'}`}>
      <div className="h-0.5 w-full" style={{ background: `linear-gradient(90deg, var(--color-teal), var(--color-teal-light), var(--color-teal))` }} />
          ...
          <div className="hidden lg:flex items-center gap-6">
            {navLinks.map(link => (
              <a key={link.name} href={link.href}
                className="text-[13px] font-medium text-[var(--color-text-mid)] transition-colors tracking-wide relative group"
                style={{ fontFamily: 'var(--font-sans)' }}>
                {link.name}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 group-hover:w-full transition-all duration-300" style={{ background: 'var(--color-teal)' }} />
              </a>
            ))}
          </div>

          <a href="tel:9718344024"
            className="hidden lg:flex btn-teal"
        ...
        <div className="lg:hidden bg-[var(--color-bg)] border-t border-gray-100 shadow-lg">
          {navLinks.map(link => (
            <a key={link.name} href={link.href}
              ...
              className="block px-6 py-3.5 text-sm text-[var(--color-text-mid)] border-b border-gray-50 transition-colors"
              style={{ fontFamily: 'var(--font-sans)' }}>
              {link.name}
            </a>
          ))}
          <div className="p-4">
            <a href="tel:9718344024" className="w-full btn-teal"
