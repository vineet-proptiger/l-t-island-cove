...
import { PROJECT_ID, PROJECT_NAME, API_ENDPOINT, SHEET_NAME, SECRET_KEY, CITY_DISPLAY } from '../../lib/lnt-island-cove-mahim/config'
...
const LeadForm = ({ formName = 'Hero Form', btnText = 'Submit Details' }) => {
  ...
  if (success) return (
    <div className="text-center py-6">
      <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: 'var(--color-teal-bg)' }}>
        <svg className="w-8 h-8" style={{ color: 'var(--color-teal-dark)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-1">
      ...
      <div className="flex items-start gap-2 mt-3">
        ...
        <label htmlFor="privacy-lead" className="text-xs text-gray-500 leading-relaxed cursor-pointer" style={{ fontFamily: F_SANS }}>
          ...
          <a href="/lnt-island-cove-mahim/privacy-policy" target="_blank" rel="noopener noreferrer" style={{ color: GOLD, textDecoration: 'underline' }}>Privacy Policy</a>
        </label>
      </div>

      <button type="submit" disabled={loading}
        className="btn-teal mt-4 w-full"
        style={{ padding: '14px', width: '80%', display: 'flex', marginLeft: 'auto', marginRight: 'auto' }}>
