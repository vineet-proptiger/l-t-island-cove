          ...
          <div className="hidden lg:flex items-center gap-6">
            {navLinks.map(link => (
              <a key={link.name} href={link.href}
                className="text-[13px] font-medium text-[var(--color-text-mid)] hover:text-teal transition-colors tracking-wide relative group"
        ...
        <div className="lg:hidden bg-[var(--color-bg)] border-t border-gray-100 shadow-lg">
          {navLinks.map(link => (
            <a key={link.name} href={link.href}
              ...
              className="block px-6 py-3.5 text-sm text-[var(--color-text-mid)] hover:bg-[var(--color-bg-light)] border-b border-gray-50 hover:text-teal transition-colors"
              style={{ fontFamily: 'var(--font-sans)' }}>
              {link.name}
            </a>
          ))}
